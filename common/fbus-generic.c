/*
  ...
  $Id: fbus-generic.c,v 1.2 2001-02-03 23:56:14 chris Exp $
...
void FBUS_RX_StateMachine(char rx_byte) {
...
#ifdef DEBUG
      fprintf(stderr,"The fbus stream is out of sync - expected 0x0c, got %2x\n",rx_byte);
#endif
    }
  ...
  case FBUS_RX_GetSource:
    ...
    if (rx_byte!=0x00)  {
      i->state=FBUS_RX_Sync;
#ifdef DEBUG
      fprintf(stderr,"The fbus stream is out of sync - expected 0x00, got %2x\n",rx_byte);
#endif
    }
  ...
  case FBUS_RX_GetMessage:
...
#ifdef DEBUG
      fprintf(stderr, "FBUS: Message buffer overun - resetting\n");
#endif
      i->state = FBUS_RX_Sync;
      break;
    }
	...
	if (i->MessageType==0x7f) {
	  ...
	  memcpy(m->MessageBuffer+m->MessageLength,
		 i->MessageBuffer,i->FrameLength-2-(i->FrameLength%2));
...
#ifdef DEBUG
		fprintf(stderr,"Received message type %02x\n\r",i->MessageType);
#endif
...
GSM_Error FBUS_SendMessage(u16 messagesize, u8 messagetype, void *message) {
  ...
  if (messagesize > FBUS_MAX_CONTENT_LENGTH) {
    ...
    for (i = 0; i < nom - 1; i++) {
      ...
      FBUS_TX_SendFrame(FBUS_MAX_CONTENT_LENGTH + 2, messagetype,
                        frame_buffer);
    ...
    FBUS_TX_SendFrame(messagesize + 2, messagetype, frame_buffer);
  }
  return (true);
}
...
#ifdef DEBUG
  fprintf(stderr, _("[Sending Ack of type %02x, seq: %x]\n"), message_type, message_seq);
#endif
...
GSM_Error FBUS_Initialise(GSM_Link *newlink, GSM_Phone *newphone)
{
  ...
  if (glink->InitLength==0) glink->InitLength=250;
  ...
  if ( glink->ConnectionType == GCT_Infrared ) {
    ...
    if (FBUS_OpenSerial() != true) return GE_DEVICEOPENFAILED;
  }
