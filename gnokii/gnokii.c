/*
  ...
  $Id: gnokii.c,v 1.449 2006-05-02 10:14:02 dforsi Exp $
  ...
  Copyright (C) 1999-2005  Pawel Kot
...
static int getfiledetailsbyid(int nargc, char *nargv[])
{
    	gn_file fi;
    	gn_file_list fil;
	gn_error error;
...
/* Delete file */
static int deletefilebyid(char *id)
{
    	gn_file fi;
	gn_error error;
...
/* Get file */
static int getfilebyid(int nargc, char *nargv[])
{
    	gn_file fi;
    	gn_file_list fil;
	gn_error error;
...
int main(int argc, char *argv[])
{
	...
	if ((nargv = malloc(sizeof(char *) * argc)) != NULL) {
		...
		case OPT_GETFILEDETAILSBYID:
			rc = getfiledetailsbyid(nargc, nargv);
			break;
		case OPT_GETFILEID:
			rc = getfileid(optarg);
			break;
